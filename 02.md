...
* What files are in a folder? The `ls` (list) command will give you a list of the files, and subfolders. Like in many Linux commands, there are options (known as "switches") to alter the meaning of the command or the output format. Try a simple  `ls`, then  `ls -l -t`  and then try  `ls -l -t -r -a`
...
* Take the time today to understand how the environment variables PS1 etc work (this article: [Bash Shell: Take Control of PS1, PS2, PS3, PS4 and PROMPT_COMMAND
